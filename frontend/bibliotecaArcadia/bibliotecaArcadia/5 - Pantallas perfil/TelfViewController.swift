...
class TelfViewController: UIViewController {
    var user: UserDB?
...
extension TelfViewController {
    //Guardar los cambios en la API
    func saveChanges() {
        //Hacer la petición a la API para guardar el cambio del número de teléfono
        let urlString = "\(API_URL)/user/phone_number"
        ...
        let bodyData = "telf=\(textfield.text!)"
            ...
            do {
                ...
                DispatchQueue.main.async {
                    ...
                    if getUserRes.success == 1 && getUserRes.user != nil {
                        //Guardar en CoreData
                        self.saveUserFieldDB(telf: getUserRes.user!.telf)
                        ...
                        self.showErrors(true, telf: getUserRes.validator!.telf?[0])
                    }
                    else {
    ...
    func saveUserFieldDB(telf: String?) {
