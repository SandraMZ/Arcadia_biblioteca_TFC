...
extension EmailViewController {
    ...
    func saveChanges() {
        //Hacer la petición a la API para guardar el cambio del email
        let urlString = "\(API_URL)/user/email"
        ...
        let bodyData = "email=\(textfield.text!)"
            ...
            do {
                ...
                DispatchQueue.main.async {
                    ...
                    if getUserRes.success == 1 && getUserRes.user != nil {
                        //Guardar en CoreData
                        self.saveUserFieldDB(email: getUserRes.user!.email)
                        
                        //ir a la pantalla anterior en la navegación
                        self.navigationController?.popViewController(animated: true)
                        
                    } else if
                        getUserRes.success == -1 && getUserRes.validator != nil {
                        //Si la respuesta de la API devuelve errores de validación, mostrarlos en el formulario
                        self.showErrors(true, email: getUserRes.validator!.email?[0])
                    }
                    else {
    ...
    func saveUserFieldDB(email: String) {
